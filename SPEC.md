...
## 11. ログ・ターン進行・テンポ・演出同期仕様追記（2026-03-01 追加）

> 対象モジュール: `turn.js`, `ai.js`, `cards.js`

1. 戦闘ログ仕様（実結果ベース）
   - ログはカード効果テキストの引用ではなく、**実際に発生した結果**を記録する。
   - 不発時は「なぜ不発だったか」を必ず明記する（例: 対象不在、マナ不足、条件未達成）。
   - 既存仕様と競合する場合、本項の実結果ベース記録を優先する。

2. ターン進行仕様（表示順序）
   - ターン開始時の表示順は、**ターン開始表示 → ドロー表示**の順で固定する。
   - 非表示処理や内部解決順が先行していても、ユーザー向け表示はこの順序を維持する。

3. テンポ仕様（AI召喚後ウェイト）
   - AIが召喚処理を完了した直後、次の行動に進む前に **0.35秒（350ms）** の待機を必ず入れる。
   - 待機は演出都合ではなくゲームテンポ仕様として保証する。

4. 演出同期仕様（非同期処理時の表示整合）
   - ドロー処理および墓地送り処理が非同期解決であっても、ログ/表示順は実際の解決順と一致させる。
   - `interactionLock` の解除条件は「処理開始直後」ではなく、**関連演出が完了した後**に統一する。

5. 参照性向上のための運用記録
   - 追加日: **2026-03-01**
   - 変更対象: `turn.js`, `ai.js`, `cards.js`
   - 本仕様項目は、上記3モジュールの今後の挙動変更時に優先参照する。
...
## 11. ターン遷移の表示順自然化（2026-03 更新）
1. ターン開始表示の順序
   - `beginTurn(owner, isNewRound)` 開始時に、`TURN n` ログと `PLAYER/ENEMY TURN` バナーを先に表示する。
   - その後にドローフェーズを処理し、ログ時系列が `TURN n` → `n枚ドロー` となるよう統一する。

2. 実装責務の分離
   - メインフェーズ開始処理を「表示専用（ヘッダー）」と「行動開始（コア）」に分離する。
   - ドロー処理は既存どおり `applyDrawPhase` が担当し、デッキアウト判定責務も維持する。

3. 敗北時遷移
   - `applyDrawPhase` で敗北が確定した場合、メイン行動フェーズへは遷移しない。

## 11. 召喚時効果ログの結果確定化（2026-03 更新）
1. `dispatchSummonEffects` のログ方針
   - 召喚時効果ログは「処理開始時」ではなく、**処理結果が確定した時点**で記録する。
   - `getEffectLogText` は効果種別ラベル用途に限定し、本文は各効果の解決結果ベースで構築する。

2. 成功／不発ログの統一
   - 成功時は「対象名」「適用値」「ドロー枚数」「消費マナ」などの具体値を記録する。
   - 不発時は「対象なし」「マナ不足」「手札なし」などの不発理由を記録する。

3. `manaGate` の明示ルール
   - 支払い成功時は `Xマナ支払い → inner効果適用` を記録する。
   - 支払い失敗時は `Xマナ不足のため不発` を記録し、inner は実行しない。

4. 対象選択ログ
   - `confirmEffectTargetChoice` 側のログは「対象選択: ...」形式から、**選択結果中心**（どの対象を指定したか）に統一する。
